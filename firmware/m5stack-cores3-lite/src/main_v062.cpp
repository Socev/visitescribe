// VisiteScribe CoreS3-Lite v0.6.2
//
// The production Olares endpoint currently presents this chain:
//   primumnonnocere.olares.com -> WR1 -> GTS Root R1 -> GlobalSign Root CA
// v0.6 used ISRG Root X1, which cannot validate that chain on the ESP32.
//
// Keep the v0.6.1 clock/NTP fix and override only the recorder-side default
// trust anchor. A local server_secrets.h can still override this macro.

#include <Arduino.h>

static const char VS_GLOBALSIGN_ROOT_R1[] PROGMEM = R"VSCA(-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)VSCA";

#ifndef VISITESCRIBE_SERVER_CA_PEM
#define VISITESCRIBE_SERVER_CA_PEM VS_GLOBALSIGN_ROOT_R1
#endif

#include "main_v061.cpp"
